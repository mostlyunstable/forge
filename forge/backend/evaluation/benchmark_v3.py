...
class BenchmarkV3:
    ...
    def evaluate_turn(self, turn_def: Dict[str, Any], turn_resp: Dict[str, Any]) -> Dict[str, float]:
    ...
    def run_benchmark(self, system_responses: Dict[str, Dict[str, Any]]) -> Dict[str, float]:
        ...
        for session in self.sessions:
            ...
            for i, turn in enumerate(session["turns"]):
                ...
                if i > 0:
                    ...
                    consistency = (session_context_scores[-1] + t_metrics["Context_Retention"]) / 2.0 if session_context_scores else t_metrics["Context_Retention"]
        ...
        if ambiguous_turns_count > 0:
            avg_metrics["Ambiguity_Handling"] = metrics_sum["Ambiguity_Handling"] / ambiguous_turns_count
        else:
            avg_metrics["Ambiguity_Handling"] = 1.0
            
        if follow_up_turns_count > 0:
            avg_metrics["Follow_Up_Understanding"] = metrics_sum["Follow_Up_Understanding"] / follow_up_turns_count
            avg_metrics["Multi_Turn_Consistency"] = metrics_sum["Multi_Turn_Consistency"] / follow_up_turns_count
        else:
            avg_metrics["Follow_Up_Understanding"] = 1.0
            avg_metrics["Multi_Turn_Consistency"] = 1.0
            
        logger.info(f"Phase 3 Benchmark Results (over {total_sessions} sessions, {total_turns} turns):")
        logger.info(json.dumps(avg_metrics, indent=2))
        
        if avg_metrics.get("Hallucination_Rate", 1.0) > 0.01:
            logger.warning(f"WARNING: Hallucination Rate ({avg_metrics['Hallucination_Rate']:.4f}) exceeds 1% threshold!")
