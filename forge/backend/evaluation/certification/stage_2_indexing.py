...
async def get_index_state(client: TestClient, project_id: str):
    ...
    try:
        points = await qdrant.query_points(
            collection_name=collection_name, query=[0.0] * 1024, limit=10000, with_payload=True
        )
...
async def async_main():
    ...
    res = client.post("/api/v1/projects", json={"name": project_name, "path": str(fixture_dir)})
    ...
    res = client.post(
        "/api/v1/index/jobs",
        json={"project_id": project_id, "repo_path": str(fixture_dir), "type": "full"},
    )
    if res.status_code != 201:
        print(f"Full index failed: {res.text}")
        sys.exit(1)

    await get_index_state(client, project_id)
    ...
    res = client.post(
        "/api/v1/index/jobs",
        json={"project_id": project_id, "repo_path": str(fixture_dir), "type": "incremental"},
    )
    if res.status_code != 201:
