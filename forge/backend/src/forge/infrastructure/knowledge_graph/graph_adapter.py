...
class SQLiteGraphAdapter(IGraphAdapter):
    ...
    def _init_db(self):
        with sqlite3.connect(self.db_path) as conn:
            ...
            """)
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_kg_source ON relationships(project_id, source_id)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_kg_target ON relationships(project_id, target_id)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_kg_type ON relationships(project_id, relationship_type)"
            )
