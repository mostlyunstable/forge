...
from forge.infrastructure.code_indexer.types import LANGUAGES, ParsedDependency, ParsedEntry
...
class TreeSitterParser:
        ...
        try:
            ...
            for parent in entries:
                children = [
                    ...
                    for c in entries
                    if c != parent
                    and c.start_line >= parent.start_line
                    and c.end_line <= parent.end_line
                ]
                if children:
                    ...
                    for child in children:
                        ...
                        if start_idx >= 0 and end_idx <= len(lines):
                            lines[start_idx:end_idx] = [
                                f"    # {child.entry_type.value} {child.name} extracted..."
                            ]
                    parent.content = "\n".join(lines)
        ...
        if node_type == "decorated_definition":
            for child in node.children:
                if child.type == "class_definition":
                    ...
                    if entry:
                        entry.content = content_bytes[node.start_byte : node.end_byte].decode(
                            "utf8"
                        )
                    ...
                    entry = self._extract_function(
                        child, content_bytes, file_path, language, parent_name
                    )
                    if entry:
                        entry.content = content_bytes[node.start_byte : node.end_byte].decode(
                            "utf8"
    ...
    def _extract_class(
        self, node: Any, content_bytes: bytes, file_path: str, language: str
    ) -> ParsedEntry | None:
    ...
    def _extract_function(
        self, node: Any, content_bytes: bytes, file_path: str, language: str, parent_name: str = ""
    ) -> ParsedEntry | None:
        name = self._get_name(node, content_bytes)
        if not name:
