class IntentRouter:
    def __init__(self):
        self.bug_keywords = {"bug", "fix", "error", "exception", "crash", "issue", "fail", "broken", "wrong", "unexpected"}
        self.arch_keywords = {"architecture", "decision", "design", "why", "structure", "pattern", "component", "system"}
        self.code_keywords = {"code", "function", "class", "method", "implement", "where is", "how does", "variable"}
        self.graph_keywords = {"dependency", "import", "depend", "caller", "called by", "impact", "relationship", "usage"}
    ...
    def route(self, query: str, context_window=None) -> dict:
        ...
        if context_window and hasattr(context_window, "messages"):
            ...
            history_text = " ".join([m.content.lower() for m in context_window.messages if hasattr(m, "content")])
        ...
        return {
            "primary_intent": max(weights.items(), key=lambda x: x[1])[0],
            "weights": weights
        }
