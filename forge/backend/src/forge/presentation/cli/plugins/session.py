...
def get_repo() -> IConversationRepository:
    return cli_container.resolve(IConversationRepository)  # type: ignore
...
def new_session(
    ...
    async def _run():
        ...
        console.print(
            Panel(
                f"Created new session: [bold cyan]{conv.id.value}[/bold cyan]", style="bold green"
            )
        )
...
@app.command("resume")
def resume_session(session_id: str = typer.Argument(..., help="ID of the session to resume")):
    """Resume a session."""
    ...
    async def _run():
        ...
        console.print(
            Panel(
                f"Resuming session: [bold cyan]{conv.title}[/bold cyan] ({session_id})",
                style="bold green",
            )
        )
...
@app.command("archive")
def archive_session(session_id: str = typer.Argument(..., help="ID of the session to archive")):
    """Archive a session."""
...
@app.command("delete")
def delete_session(session_id: str = typer.Argument(..., help="ID of the session to delete")):
    """Delete a session."""
    ...
    async def _run():
        ...
        if success:
            ...
            console.print(
                f"[bold red]Session {session_id} not found or could not be deleted.[/bold red]"
            )
