...
class ConversationContextManager:
    ...
    async def build_context(
        ...
        retrieved_tokens = sum(
            self._token_manager.estimate_tokens(ctx.content) for ctx in deduped
        )
        ...
        context_window = self._token_manager.build_context_window(
            ...
            memory_tokens=retrieved_tokens
        )
    ...
    def _deduplicate_and_sort_contexts(
        ...
        deduped.sort(key=lambda x: (-x.score, hashlib.sha256(x.content.encode('utf-8')).hexdigest()))
        return deduped
