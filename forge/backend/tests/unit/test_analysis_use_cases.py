...
class TestRiskCalculator:
    def test_low_risk(self):
        cs = ChangeSet(entries=[
            ChangeEntry(file_path="tests/test_foo.py", change_type=ChangeType.ADDED, lines_added=10, is_test_file=True),
        ])
    ...
    def test_high_risk_domain_changes(self):
        cs = ChangeSet(entries=[
            ChangeEntry(file_path="src/forge/domain/entities/project.py", change_type=ChangeType.MODIFIED, lines_added=100, lines_removed=50, is_core_module=True),
            ChangeEntry(file_path="src/forge/domain/value_objects/id.py", change_type=ChangeType.MODIFIED, lines_added=50, lines_removed=20, is_core_module=True),
        ])
        impact = DependencyImpact(
        ...
        history = HistoricalContext(
            related_bugs=[RelatedBug(id="1", title="Bug", root_cause="X", solution="Y", severity="high", resolved=False)]
        )
        risk = calculate_risk(cs, impact, history)
...
class TestRecommendations:
    def test_domain_change_recommendation(self):
        cs = ChangeSet(entries=[
            ChangeEntry(file_path="src/forge/domain/entities/project.py", change_type=ChangeType.MODIFIED),
        ])
        impact = DependencyImpact()
        history = HistoricalContext()
    ...
    def test_no_tests_recommendation(self):
        cs = ChangeSet(entries=[
            ChangeEntry(file_path="src/forge/domain/entities/x.py", change_type=ChangeType.MODIFIED),
        ])
        impact = DependencyImpact()
        history = HistoricalContext()
        risk = RiskAssessment(score=30, level=RiskLevel.LOW)
    ...
    def test_historical_bugs_recommendation(self):
        cs = ChangeSet(entries=[ChangeEntry(file_path="a.py", change_type=ChangeType.MODIFIED)])
        impact = DependencyImpact()
        history = HistoricalContext(related_bugs=[RelatedBug(id="1", title="B", root_cause="X", solution="Y", severity="high", resolved=False)])
        risk = RiskAssessment(score=30, level=RiskLevel.LOW)
        recs = generate_recommendations(cs, impact, history, risk)
