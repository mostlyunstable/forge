...
async def test_context_manager_compression():
    ...
    msg1 = ConversationMessage.create_user(conv_id, content="First very long user message here", token_count=10)
    msg2 = ConversationMessage.create_assistant(conv_id, content="Assistant reply to the first message", token_count=10)
    msg3 = ConversationMessage.create_user(conv_id, content="Second user message here", token_count=10)
    conv.add_message(msg1)
    conv.add_message(msg2)
    conv.add_message(msg3)

    summary = ConversationSummary.create(conv_id, content="Summary of the conversation", token_count=10)
    conv.add_summary(summary)
