...
class TestUpdateMemoryUseCase:
    ...
    async def test_update_memory_creates_new_version(self, memory_repo_mock, sample_note):
        ...
        request = UpdateMemoryRequest(
            memory_id=str(sample_note.id.value), title="New Title", summary="New Summary"
        )
...
class TestSearchKnowledgeUseCase:
    ...
    async def test_search_knowledge_matches(self, memory_repo_mock, sample_note):
        ...
        request = SearchKnowledgeRequest(
            project_id=str(sample_note.project_id.value), query="original", limit=10
        )
    ...
    @pytest.mark.asyncio
    async def test_search_knowledge_ignores_archived_and_superseded(
        self, memory_repo_mock, sample_note
    ):
