...
from forge.domain.memory.exceptions import DecisionNotFoundError, BugNotFoundError
...
class TestSaveDecisionUseCase:
    ...
    async def test_save_decision_project_not_found(self, decision_repo, project_repo):
        ...
        with pytest.raises(ProjectNotFoundError):
            await use_case.execute(SaveDecisionRequest(
    ...
    @pytest.mark.asyncio
    async def test_save_decision_publishes_event(self, decision_repo, project_repo, sample_project, event_bus):
...
class TestListDecisionsUseCase:
    @pytest.mark.asyncio
    async def test_list_decisions(self, decision_repo, project_repo, sample_project, sample_decision):
...
class TestUpdateDecisionUseCase:
    ...
    async def test_update_decision(self, decision_repo, sample_decision):
        ...
        result = await use_case.execute(UpdateDecisionRequest(
    ...
    async def test_update_decision_not_found(self, decision_repo):
        use_case = UpdateDecisionUseCase(decision_repo)
        with pytest.raises(DecisionNotFoundError):
            await use_case.execute(UpdateDecisionRequest(
...
class TestSaveBugUseCase:
    ...
    async def test_save_bug(self, bug_repo, project_repo, sample_project):
        ...
        result = await use_case.execute(SaveBugRequest(
            project_id=str(sample_project.id.value),
            title="SQL injection",
    ...
    @pytest.mark.asyncio
    async def test_save_bug_publishes_event(self, bug_repo, project_repo, sample_project, event_bus):
        use_case = SaveBugUseCase(bug_repo, project_repo, event_bus=event_bus)
        await use_case.execute(SaveBugRequest(
            project_id=str(sample_project.id.value),
            title="Event Test",
            problem="Test",
            root_cause="Test",
...
class TestUpdateBugUseCase:
    ...
    async def test_update_bug(self, bug_repo, sample_bug):
        ...
        result = await use_case.execute(UpdateBugRequest(
...
class TestSavePreferenceUseCase:
    ...
    async def test_save_new_preference(self, preference_repo):
        ...
        result = await use_case.execute(SavePreferenceRequest(
            key="testing",
            value="pytest",
            confidence=0.9,
        ))
