...
class TestProjectRepository:
    ...
    async def test_search_by_name(self, db_session):
        repo = ProjectRepository(db_session)
        await repo.save(
            Project.create(name="Alpha Project", description="", stack=TechStack.from_list([]))
        )
        await repo.save(
            Project.create(name="Beta Project", description="", stack=TechStack.from_list([]))
        )
