...
@pytest.mark.asyncio
async def test_planning_engine_injects_system_prompt_with_read_only_constraints(planning_engine, mock_llm_provider):
    # Arrange
    mock_llm_provider.chat_mock.return_value = "Structured Plan"
    ...
    assert "you NEVER modify repositories, generate commits, or execute changes automatically" in called_messages[0]["content"]
    assert "You MUST provide recommendations strictly based on retrieved evidence." in called_messages[0]["content"]
    assert "Retrieved Context:\nThis is a piece of evidence for planning." in called_messages[0]["content"]
...
@pytest.mark.asyncio
async def test_planning_engine_verifies_output_format_instructions(planning_engine, mock_llm_provider):
    # Arrange
    mock_llm_provider.chat_mock.return_value = "Plan"
    context_window = ContextWindow(
        summary="",
        summary_tokens=0,
        messages=[],
    ...
    assert "implementation steps, debugging strategies, migration planning, testing plans, architecture comparisons, and refactoring recommendations" in system_prompt
