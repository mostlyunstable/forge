...
class MockLLMProvider(ILLMProvider):
    def __init__(self):
        ...
        self.chat_mock.return_value = LLMResponse(content="Response", model="test", usage={})

    async def chat(self, messages: list[dict[str, str]], **kwargs) -> LLMResponse:
        return await self.chat_mock(messages, **kwargs)

    async def chat_stream(self, messages: list[dict[str, str]], **kwargs):
        yield {"type": "text", "content": "Streamed validation"}
...
async def test_planning_engine_declines_modification_intent(planning_engine, mock_llm_provider):
    ...
    mock_llm_provider.chat_mock.return_value = LLMResponse(
        content="I am a read-only planning assistant.", model="test", usage={}
    )
    ...
    await planning_engine.generate_plan(
        query=malicious_query, context_window=context_window, retrieved_context=""
    )
...
@pytest.mark.asyncio
async def test_reasoning_engine_protects_against_prompt_injection(
    ...
    await reasoning_engine.generate_response(
        context_window=context_window, retrieved_context="", user_prompt=malicious_prompt
    )
    ...
    assert "You never fabricate architecture, code, or history" in system_prompt
...
async def test_context_window_isolation_across_sessions(reasoning_engine, mock_llm_provider):
    ...
    context_window_a = ContextWindow(
        summary="", summary_tokens=0, messages=[msg_a], message_tokens=10, total_tokens=10
    )

    await reasoning_engine.generate_response(
        context_window=context_window_a, retrieved_context="", user_prompt="What is my secret?"
    )
    ...
    context_window_b = ContextWindow(
        summary="", summary_tokens=0, messages=[msg_b], message_tokens=10, total_tokens=10
    )

    await reasoning_engine.generate_response(
        context_window=context_window_b, retrieved_context="", user_prompt="What is my secret?"
    )
